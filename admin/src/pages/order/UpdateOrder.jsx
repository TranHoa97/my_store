...
const UpdateOrder = (props) => {
    ...
    useEffect(() => {
        ...
        const data = results.details?.map((item) => {
            const select = variants.find(e => e.variants_id === item.variants_id && e.color === item.color)
            return {
                index: select?.value,
                quantity: Number(item.quantity),
                rate: Number(item.price) / Number(item.quantity),
        ...
        form.setFieldValue('products', data);
        setAmount(results.details?.reduce((total, item) => total + Number(item.price), 0))
    }, [])
